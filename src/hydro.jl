...
function con2prim(cons::SVector{4})
    Ne = 0.0
    xlo = SVector(0.001, 0.1, 0.0)
    xhi = SVector(10.0, 3.0, 3.0)
    function f(x)
        local ρ, S, w = x
        local prims1 = SVector(ρ, S, Ne, w)
        local cons1 = prim2con(prims1)
        local res = cons1 - cons
        local D, De, M, T = res
        return SVector(D, M, T)
    end
    sol = solve3d(f, xlo, xhi)
    @show sol
    return sol.x
end
